...
export async function GET() {
  // Teste no navegador: http://localhost:3000/api/checkout
...
export async function POST() {
  try {
    ...
    if (!siteUrl) {
      return Response.json({ error: "NEXT_PUBLIC_SITE_URL ausente" }, { status: 500 });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      payment_method_types: ["card"], // Apple/Google Pay aparecem no Checkout
      line_items: [
        {
          price_data: {
            currency: "eur",
            unit_amount: 99, // €0,99 (centavos)
